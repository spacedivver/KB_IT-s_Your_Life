		...
		<h2>게시물 수정</h2>
 	    <form action="<%= request.getContextPath()%>/board/update" method="POST" enctype="multipart/form-data"> 
			...
			<%if(board.getOriginalFilename() != null && board.getOriginalFilename().length() > 0){%>
    			<input type="hidden" name="original_filename" value="<%=board.getOriginalFilename()%>"/>
				<input type="hidden" name="renamed_filename" value="<%=board.getRenamedFilename()%>"/>
			<%}%>
			<table id='tbl-board'>
				<tr>
					<th>제목</th>
				...
				<tr>
					...
					<td>
						...
						<%if(board.getOriginalFilename() != null && board.getOriginalFilename().length() > 0){%>
			    			<br><img src="<%=request.getContextPath()%>/resources/images/file.png" width="20" height="20"/>
