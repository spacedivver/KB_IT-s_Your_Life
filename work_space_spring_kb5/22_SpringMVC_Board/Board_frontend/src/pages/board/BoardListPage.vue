...
<!-- 
    <div class="mt-4">
      ...
      <div v-for="item in page.boardCategory" :key="item.type" class="form-check form-check-inline">
        <input @change="typeChange" checked class="form-check-input" type="checkbox" :id="item.type" :value="item.type">
    ...
    <div class="row mt-2">
      ...
      <div class="col-8">
        ...
        <input v-model="pageRequest.searchValue" v-on:keyup.enter="load(pageRequest)" type="text" class="form-control" placeholder="내용을 입력하세요.">
      </div>
      <div class="col-1">
        <label  class="form-label"> &nbsp <span class="text-body-secondary"></span></label>
        <button class="form-control btn btn-primary" @click="load(pageRequest)">검색</button>
      </div>
    </div>
